...
ssh pi@192.168.1.99 'sudo apt-get install -y s3fs;sudo rm -r /etc/fuse.conf;sudo mv ~/fuse.conf /etc/;sudo umount ${HOME}/clusterdata;sudo rm -r clusterdata;chmod 600 ${HOME}/.passwd-s3fs;mkdir ${HOME}/clusterdata;s3fs raspberrycluster ${HOME}/clusterdata -o allow_other -o passwd_file=${HOME}/.passwd-s3fs -o url="https://s3-eu-central-1.amazonaws.com";exit'
ssh pi@192.168.1.98 'sudo apt-get install -y s3fs;sudo rm -r /etc/fuse.conf;sudo mv ~/fuse.conf /etc/;sudo umount ${HOME}/clusterdata;sudo rm -r clusterdata;chmod 600 ${HOME}/.passwd-s3fs;mkdir ${HOME}/clusterdata;cat /etc/fuse.conf;s3fs raspberrycluster ${HOME}/clusterdata -o allow_other -o passwd_file=${HOME}/.passwd-s3fs -o url="https://s3-eu-central-1.amazonaws.com";exit'
